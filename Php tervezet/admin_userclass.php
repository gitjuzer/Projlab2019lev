<?php require 'config.php'; user(5); 
	...
	// Fájlnév
	define('FILENAME', 'admin_userclass.php');
	
	// Akció
	if (isset($_POST['action_edit'])) {
		// update
		query(sql('user:classEdit', array('id'=>$_POST['id'],'name'=>$_POST['name'])),'array');
		echo 'Módosítás megtörtént. <a href="'.FILENAME.'">Tovább</a>'; exit();
	} else if (isset($_POST['action_new'])) {
		// insertinto
		query(sql('user:classNew', array('name'=>$_POST['name'])),'array');
	...
	// HTML fejléc
	html('header', array(
	    'title'=>'Osztályok kezelése'.($_GET['a']=='new'?' - Új létrehozása':($_GET['a']=='edit'?' - Szerkesztés':'')), 
...
<?php
	...
	if (!isset($_GET['a'])) {
		...
		$coursecats = query(sql('user:classAll'),'array');

		echo '<style>td,th{border:1px solid #000;padding:6px;}</style>';
		echo '<table align="center"><tr>'.
			'<th>Az.</th><th>Megenvezés</th><th>Műveletek</th></tr>';
		...
		// szerk|új
		if (isset($_GET['id'])) {
			$coursecatinfo = query(sql('user:classInfo', array('id'=>$_GET['id'])),'array')[0];
		}
?>
